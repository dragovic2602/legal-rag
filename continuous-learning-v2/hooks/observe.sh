...
if [ "$OBSERVER_ENABLED" = "true" ]; then
  ...
  if [ ! -f "${PROJECT_DIR}/.observer.pid" ] && [ ! -f "${CONFIG_DIR}/.observer.pid" ]; then
    ...
    if command -v flock >/dev/null 2>&1; then
      ...
      # macOS fallback: use lockfile if available
      if command -v lockfile >/dev/null 2>&1; then
        ...
        (
          ...
          _CHECK_OBSERVER_RUNNING "${CONFIG_DIR}/.observer.pid" || true
          if [ ! -f "${PROJECT_DIR}/.observer.pid" ] && [ ! -f "${CONFIG_DIR}/.observer.pid" ]; then
            ECC_OBSERVER_ALLOW_WINDOWS=true nohup "${SKILL_ROOT}/agents/start-observer.sh" start >/dev/null 2>&1 &
          fi
          rm -f "$LAZY_START_LOCK" 2>/dev/null || true
        )
      else
        # Windows/Git-Bash fallback: mkdir is atomic on all platforms
        if mkdir "${LAZY_START_LOCK}.d" 2>/dev/null; then
          _CHECK_OBSERVER_RUNNING "${PROJECT_DIR}/.observer.pid" || true
          _CHECK_OBSERVER_RUNNING "${CONFIG_DIR}/.observer.pid" || true
          if [ ! -f "${PROJECT_DIR}/.observer.pid" ] && [ ! -f "${CONFIG_DIR}/.observer.pid" ]; then
            ECC_OBSERVER_ALLOW_WINDOWS=true nohup "${SKILL_ROOT}/agents/start-observer.sh" start >/dev/null 2>&1 &
          fi
          rmdir "${LAZY_START_LOCK}.d" 2>/dev/null || true
        fi
      fi
    fi
  fi
