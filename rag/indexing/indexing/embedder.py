...
from generation.providers import get_embedding_client, get_embedding_model
...
class EmbeddingGenerator:
    ...
    async def embed_chunks(
        ...
        for i in range(0, len(chunks), self.batch_size):
            ...
            # Build context-prefixed texts for embedding (content stored in DB unchanged)
            batch_texts = []
            for chunk in batch_chunks:
                title = chunk.metadata.get("title", "")
                heading = chunk.metadata.get("headings", "")
                if title or heading:
                    prefix = " > ".join(filter(None, [title, heading]))
