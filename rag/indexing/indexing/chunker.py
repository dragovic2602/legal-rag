...
# Heavy ML imports are deferred to DoclingHybridChunker.__init__ to avoid
# slow startup when the module is first imported.
DoclingDocument = None  # will be set at runtime if needed
...
class DoclingHybridChunker:
    ...
    def __init__(self, config: ChunkingConfig):
        ...
        # Lazy-load heavy ML libraries here (not at module level) so that
        # importing this module doesn't add 30-60s of startup time.
        print("Loading tokenizer and HybridChunker (first run may take a moment)...", flush=True)
    ...
    async def chunk_document(
        ...
        try:
            ...
            # Post-split any chunk that exceeds max_tokens (happens when Docling
            # produces flat/unstructured text blocks from scanned PDFs)
            final_chunks = []
            for chunk in document_chunks:
                if chunk.token_count > self.config.max_tokens:
                    sub = self._split_large_chunk(chunk)
            ...
            logger.info(f"Created {len(final_chunks)} chunks using HybridChunker")
            return final_chunks
    ...
    def _split_large_chunk(self, chunk: "DocumentChunk") -> List["DocumentChunk"]:
        """Split a single oversized chunk using a token-aware sliding window."""
        text = chunk.content
        tokens = self.tokenizer.encode(text)
        max_t = self.config.max_tokens
        overlap = max_t // 5  # 20% overlap

        sub_chunks = []
        start = 0
        while start < len(tokens):
            end = min(start + max_t, len(tokens))
            sub_text = self.tokenizer.decode(tokens[start:end], skip_special_tokens=True)
            sub_chunks.append(DocumentChunk(
                content=sub_text.strip(),
                index=len(sub_chunks),
                start_char=chunk.start_char,
                end_char=chunk.end_char,
                metadata={**chunk.metadata, "chunk_method": "hybrid+split"},
